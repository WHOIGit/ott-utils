...
from ..legacy import MatClassSummary
from ..class_summary import ClassSummary

LEGACY_SUMMARY_FILE = os.path.join('ott','tests','data','summary_allTB2006.mat')

class TestLegacyClassSummary(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        mcs = MatClassSummary(LEGACY_SUMMARY_FILE)
        cls.cs = mcs.to_class_summary()
        cls.cs_adhoc = mcs.to_class_summary(thresh='adhoc')
        cls.cs_opt = mcs.to_class_summary(thresh='opt')
    def test_timestamps(self):
        ts = self.cs.timestamps()[0]
    ...
    def test_conc_opt_binning(self):
        df = self.cs_opt.concentrations(frequency='2d')
